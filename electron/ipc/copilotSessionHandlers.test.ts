...
describe('copilotSessionHandlers', () => {
  ...
  it('copilot-sessions:compute-digest returns null when session is null', async () => {
    const { getSessionDetail } = await import('../services/copilotSessionService')
    vi.mocked(getSessionDetail).mockResolvedValueOnce(null as never)
    const handler = handlers.get('copilot-sessions:compute-digest')!
    const storagePath = '/home/user/.config/Code/User/workspaceStorage'
    const result = await handler({}, `${storagePath}/ws123/session.jsonl`)
    expect(result).toBeNull()
  })
  ...
  it('copilot-sessions:compute-digest returns digest for valid session', async () => {
    const handler = handlers.get('copilot-sessions:compute-digest')!
    const storagePath = '/home/user/.config/Code/User/workspaceStorage'
    const result = await handler({}, `${storagePath}/ws123/sub/session.jsonl`)
