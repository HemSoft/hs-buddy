...
function PRTableRow({
  ...
  pr: RepoPullRequest
  onClick: (pr: RepoPullRequest) => void
}) {
  return (
    <tr key={pr.number} onClick={() => onClick(pr)}>
  ...
  return (
    <div className="repo-prs-list" style={{ display: 'block', padding: 0 }}>
      <table className="list-view-table">
        ...
        <tbody>
          {prs.map(pr => (
            <PRTableRow key={pr.number} pr={pr} onClick={handlePRClick} />
          ))}
        </tbody>
      </table>
    </div>
...
function PRCardApprovals({ pr }: { pr: RepoPullRequest }) {
