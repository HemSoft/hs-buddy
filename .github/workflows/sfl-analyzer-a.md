...
on:
  ...
  workflow_dispatch:
    inputs:
      pull-request-number:
        description: Target draft PR number for a targeted Analyzer A review
        required: false
...
safe-outputs:
  ...
  dispatch-workflow:
    ...
    max: 1
---
source: relias-engineering/set-it-free-loop/workflows/sfl-analyzer-a.md@79100291d171fa15d82a21338d23a2cf4f6063b6
...
- For Analyzer B dispatch, use `safeoutputs-sfl_analyzer_b`
...
{{else}}
No `pull-request-number` input was provided (e.g., `pull_request` trigger or
manual `workflow_dispatch` without input). Search for open pull requests in
this repository that meet ALL criteria:
{{/if}}
...
"PR #<number> already reviewed by Analyzer A in cycle <N> — skipping.",
...
- `body`: a concise description of the blocking issue, prefixed with
  `:red_circle: **BLOCKING (Analyzer A)**:` so the source is clear
...
- `body`: `YYYY-MM-DD h:mm AM/PM EDT | SFL Analyzer A | :fast_forward: SKIPPED — <reason>` where `<reason>` is a short phrase like "no eligible PRs", "already reviewed", "broken handoff", or "duplicate PRs"
...
1. `add_comment` posting the current-cycle Analyzer A review as a PR comment
...
5. `dispatch_workflow` invoking `sfl-analyzer-b` with `pull-request-number: <number>`

Do not stop after step 1. A review run is incomplete unless all required actions happen.

## Step 7 — Dispatch Analyzer B

After posting the review comment and activity log, dispatch `sfl-analyzer-b`
with input `pull-request-number: <number>`.

Do NOT decide whether the PR should pass or fail. Analyzer B will review
next in the chain.

Use this exact action order for a normal review run:

- First: `add_comment` with the Analyzer A review as a PR comment
...
- Fifth: `dispatch_workflow` to `sfl-analyzer-b` with `pull-request-number`
...
- `dispatch_workflow(workflow="sfl-analyzer-b", inputs={"pull-request-number":"<number>"})`

Invalid completion patterns:

- `add_comment(...)` and then stop
- `add_comment(...)` + `add_comment(...)` and then stop
- `add_comment(...)` + plain text saying Analyzer B should run next

If you are about to finish the run and have not yet dispatched `sfl-analyzer-b`, stop and dispatch it before producing any final text.
...
- Never modify PR code or draft status — only post review comments and dispatch Analyzer B
