...
function IssueCommentItem({ comment }: { comment: RepoIssueDetail['comments'][number] }) {
  /* v8 ignore next */
  const commentDate = comment.updatedAt > comment.createdAt ? comment.updatedAt : comment.createdAt
  return (
    <article className="repo-issue-detail-comment">
      <div className="repo-issue-detail-comment-head">
        <div className="repo-issue-detail-comment-author">
          {comment.authorAvatarUrl && <img src={comment.authorAvatarUrl} alt={comment.author} />}
          <div>
