...
export function RepoNode({
  ...
  const isIssueSelected = selectedItem === `repo-issues:${repoKey}` && !isIssuesGroupExpanded
  ...
  const isPRSelected = selectedItem === `repo-prs:${repoKey}` && !isPRGroupExpanded
        ...
        <div className="sidebar-repo-children">
          ...
          >
            ...
            {isCommitLoading ? (
              ...
              <span className="sidebar-item-count">{Math.min(repoCommitTreeData[repoKey].length, 25)}</span>
            ) : null}
          </div>
          ...
          >
            ...
            >
              {expandedRepoIssueGroups.has(repoKey) ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
            </span>
            <span className="sidebar-item-icon">
              <CircleDot size={12} />
            </span>
          ...
          {(!sflStatus || !sflStatus.isSFLEnabled) && isSFLLoading && (
                  ...
                  <div className="sidebar-job-items">
                    ...
                    {sflStatus.workflows.map(wf => (
                      ...
                      >
                        ...
                        {wf.state !== 'active' && (
                          <span className="sidebar-sfl-disabled-badge">off</span>
                        )}
                      </div>
                    ))}
                  </div>
