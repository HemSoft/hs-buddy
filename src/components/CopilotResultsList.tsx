...
function filterCopilotResultsByStatus(
  results: NonNullable<ReturnType<typeof useCopilotResultsRecent>> | undefined,
  statusFilter: string
) {
  if (!results) return []
  return statusFilter === 'all' ? results : results.filter(r => r.status === statusFilter)
}
  ...
  return (
    <tr className="copilot-result-row" onClick={() => onOpenResult(r._id)}>
      ...
      <td className="result-actions-cell">
        {r.category === 'pr-review' && <PrLinkButton metadata={metadata} />}
        <button
...
export function CopilotResultsList({ onOpenResult }: CopilotResultsListProps) {
  ...
  const filteredResults = filterCopilotResultsByStatus(results, statusFilter)
