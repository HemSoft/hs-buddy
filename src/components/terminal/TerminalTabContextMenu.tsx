...
export function TerminalTabContextMenu({
  x, y, tab, onRename, onSetColor, onOpenFolderView, onClose,
      ...
      <div className="terminal-ctx-menu" style={{ top: y, left: x }} ref={menuRef}>
        {mode === 'menu' ? (
          <>
            ...
            {tab.cwd && (
              <button className="terminal-ctx-item" onClick={() => { onOpenFolderView(tab.cwd); onClose() }}>
                <FolderOpen size={14} />Folder View
              </button>
            )}
            <div className="terminal-ctx-separator" />
            <div className="terminal-ctx-label"><Palette size={12} />Color</div>
            <div className="terminal-ctx-colors">
              {TAB_COLORS.map(c => (
                <button key={c.value}
                  className={`terminal-ctx-color-swatch ${tab.color === c.value ? 'active' : ''}`}
                  style={{ backgroundColor: c.value }} title={c.name}
                  onClick={() => { onSetColor(tab.id, c.value); onClose() }} />
              ))}
              {tab.color && (
                <button className="terminal-ctx-color-reset" title="Reset color"
                  onClick={() => { onSetColor(tab.id, undefined); onClose() }}><X size={10} /></button>
              )}
            </div>
          </>
        ) : (
          <div className="terminal-ctx-rename">
            <input ref={inputRef} className="terminal-ctx-rename-input" value={renameValue}
              onChange={e => setRenameValue(e.target.value)} onKeyDown={handleRenameKeyDown}
              onBlur={handleRenameSubmit} maxLength={40} />
          </div>
        )}
      </div>
    </>
