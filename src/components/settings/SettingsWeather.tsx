...
export function SettingsWeather() {
  ...
  useEffect(() => {
    window.ipcRenderer.invoke(IPC_INVOKE.CONFIG_GET_POLLEN_API_KEY)
      .then((key: string) => { setApiKey(key || ''); setSavedKey(key || '') })
      .catch(() => {}).finally(() => setLoading(false))
  }, [])

  const handleSave = async () => {
    setSaving(true)
    try { await window.ipcRenderer.invoke(IPC_INVOKE.CONFIG_SET_POLLEN_API_KEY, apiKey.trim()); setSavedKey(apiKey.trim()) }
    catch (_: unknown) { /* IPC error */ }
    finally { setSaving(false) }
  }
  const handleClear = async () => {
    setSaving(true)
    try { await window.ipcRenderer.invoke(IPC_INVOKE.CONFIG_SET_POLLEN_API_KEY, ''); setApiKey(''); setSavedKey('') }
    catch (_: unknown) { /* IPC error */ }
    finally { setSaving(false) }
  }
  ...
  if (loading) {
    return (<div className="settings-page"><div className="settings-loading">
      <RefreshCw className="spin" size={24} /><p>Loading weather settings...</p>
    </div></div>)
  }
    ...
    <div className="settings-page">
      ...
      <div className="settings-page-content">
        <div className="settings-section">
          <div className="section-header"><h3><Key size={16} />Pollen API Key</h3></div>
          ...
          <ApiKeyInputRow apiKey={apiKey} showKey={showKey} onApiKeyChange={setApiKey} onToggleShow={() => setShowKey(!showKey)} />
          <SaveButtonGroup saving={saving} isDirty={isDirty} apiKey={apiKey} isConfigured={isConfigured} onSave={handleSave} onClear={handleClear} />
          <ConfigStatusIndicator isConfigured={isConfigured} />
          <div className="info-box">
            <p><strong>Get a free API key:</strong></p>
            <p>
              <a href={pollenUrl} className="settings-link" onClick={e => { e.preventDefault(); window.ipcRenderer.invoke('shell:open-external', pollenUrl) }}>
                <ExternalLink size={12} />Enable the Google Pollen API
              </a>{' '}— enable the API in your Google Cloud project, then create an API key under Credentials.
            </p>
            <p className="mt-2">
