...
| B | **Discussion Label Trigger** | When a discussion is labeled `action-item`, the `discussion-processor` fires and creates issues |
...
| `agent:fixable` | 🟢 `#2ea44f` | Agent has analyzed this and can auto-fix safely | discussion-processor, human | issue-processor (via dispatcher) |
| `agent:in-progress` | 🔵 `#0075ca` | Agent is actively working on this item | issue-processor | sfl-auditor |
| `agent:human-required` | 🔴 `#d73a4a` | Fix exceeds safe automation boundary — human must own | issue-processor, sfl-auditor | human |
...
| `no-agent` | ⚪ `#ffffff` | Opt this issue out of all SFL automation | human | all workflows (skip) |

### Lifecycle Labels (PR)

| Label | Color | Description | Applied By | Consumed By |
|-------|-------|-------------|-----------|-------------|
| `agent:pr` | ⬜ `#ededed` | Marks a PR as SFL-managed | issue-processor | analyzers, fixer, promoter, dispatcher |
| `agent:promoted` | 🔵 `#1D76DB` | PR promoted from draft to ready-for-review | pr-promoter | human (informational) |
| `pr:cycle-1` … `pr:cycle-12` | 🔵→⬜ gradient | Tracks how many analyze→fix cycles the PR has been through | pr-fixer | dispatcher, promoter |
...
| `audit` | ⬜ `#ededed` | Produced by an audit workflow |
| `source:repo-audit` | 🔵 `#bfd4f2` | Generated by repo-audit |
| `agentic-workflows` | ⬜ `#ededed` | Related to agentic workflow infrastructure |
| `bug` | 🔴 `#d73a4a` | Something isn't working |
| `documentation` | 🔵 `#0075ca` | Improvements or additions to documentation |
| `duplicate` | ⬜ `#cfd3d7` | This issue or pull request already exists |
| `invalid` | 🟡 `#e4e669` | This doesn't seem right |
| `daily-status` | ⬜ `#ededed` | Daily status report |
...
| **Safe-Outputs** | `create-issue` (max 10, prefix `[simplisticate]`, labels: `report`, `audit`), `update-issue` (max 5), `add-comment` (max 1) |
| **Input** | Full repository codebase |
| **Output** | Summary report issue + individual `agent:fixable` issues |
| **Artifact State** | `010-issue-created` |

#### `daily-repo-status` — Daily Status Report
...
| **Output** | Daily status Discussion |
...
#### `discussion-processor` — Discussion → Issues
...
#### `issue-processor` — Claim Issue, Open Draft PR
...
| **Label Transitions** | Issue: `agent:fixable` → `agent:in-progress`; PR: gets `agent:pr` + `pr:cycle-0` (implicit) |
...
#### `pr-label-actions` — Label-Driven PR Operations

| Field | Value |
|-------|-------|
| **Type** | Standard YAML (no AI inference) |
| **Trigger** | `pull_request: [labeled]`, `workflow_dispatch` (dispatcher) |
| **Permissions** | `pull-requests: write`, `contents: write`, `discussions: write` |
| **Operations** | `human:ready-for-review` on draft → flips to non-draft; `ready-to-merge` on non-draft → squash-merge + CHANGELOG update + branch delete |
...
════════════════════════════
  ...
  └────────┬────────┘    └──────────┬──────────┘
    ...
    Labels: (none yet)         Labels: report, audit
           │                        │
           │   ┌────────────────────┘
...
════════════════════════════════════════
           ...
           ▼
    ┌─────────────────────┐
    │ discussion-processor │    ← Triggered by `action-item` label on discussion
    │  (event: labeled)    │
    └────────┬─────────────┘
...
══════════════════════════════════
    ...
    │  guard runs first │
    └────────┬──────────┘
             │  Dispatches issue-processor when agent:fixable issues exist
             ▼

PHASE 4 — ISSUE → PR (Dispatcher-triggered)
════════════════════════════════════════════
    ┌──────────────────┐
    │ issue-processor   │    ← Claims oldest agent:fixable issue
...
| `020-issue-ready` | Issue triaged with `agent:fixable` or `agent:human-required` | Issue with lifecycle label | `030-pr-ready-for-review` (via issue-processor) |
...
| `040-pr-ready-for-human-review` | PR un-drafted, all analyzers PASS, awaiting human | Non-draft PR with `human:ready-for-review` | **Merged** (via pr-promoter or pr-label-actions) |
...
```
agent:pr + pr:cycle-0 (implicit)
  → [analyzers review] → pr:cycle-0 markers written
