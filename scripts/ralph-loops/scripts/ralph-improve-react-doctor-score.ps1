...
if ($Help) {
    ...
    Write-Host "  -Branch <name>         Override branch name (default: feature/improve-react-doctor)"
...
$defaultPrompt = "Improve the react-doctor results to return 100"
$effectivePrompt = if ($Prompt) { $Prompt } else { $defaultPrompt }
$effectiveBranch = if ($Branch) { $Branch } else { 'feature/improve-react-doctor' }
