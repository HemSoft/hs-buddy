...
| `source:repo-audit` | `#bfd4f2` (light blue) | Generated by repo-audit workflow |
...
[detected] → agent:fixable
           → agent:in-progress  (issue processor claims it, creates PR with agent:pr)
           → PR enters analyzer chain (a → b → c → label-actions)
...
Fix cycle (analyzer:blocked):
  label-actions removes analyzer:blocked → dispatches issue-processor
  → issue-processor pushes fix → increments pr:cycle-N
  → re-enters analyzer chain
